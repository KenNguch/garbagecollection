		...
		$pcConfig = array(
			...
			'availability' => array(   
				'truck' => array(   
					...
					'sortable-fields' => array(0 => '`availability`.`id`', 1 => '`trucks1`.`number`', 2 => 3, 3 => '`routes1`.`amount`', 4 => '`availability`.`date`', 5 => '`availability`.`time`', 6 => 7),
					'records-per-page' => 10,
					'default-sort-by' => 0,
					...
					'query' => "SELECT `availability`.`id` as 'id', IF(    CHAR_LENGTH(`trucks1`.`number`), CONCAT_WS('',   `trucks1`.`number`), '') as 'truck', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS('',   `routes1`.`name`, '  :', `routes1`.`time`), '') as 'route', IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS('',   `routes1`.`amount`), '') as 'amount', if(`availability`.`date`,date_format(`availability`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`availability`.`time`, '%r') as 'time', `availability`.`status` as 'status' FROM `availability` LEFT JOIN `trucks` as trucks1 ON `trucks1`.`id`=`availability`.`truck` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` "
				),
				'route' => array(   
					'parent-table' => 'routes',
					'parent-primary-key' => 'id',
			...
			'bookings' => array(   
				'id_number' => array(   
					...
					'query' => "SELECT `bookings`.`id` as 'id', IF(    CHAR_LENGTH(`customers1`.`id_number`), CONCAT_WS('',   `customers1`.`id_number`), '') as 'id_number', IF(    CHAR_LENGTH(`customers1`.`fullname`), CONCAT_WS('',   `customers1`.`fullname`), '') as 'fullname', IF(    CHAR_LENGTH(`customers1`.`phone`), CONCAT_WS('',   `customers1`.`phone`), '') as 'phone', IF(    CHAR_LENGTH(`trucks1`.`number`) || CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS('',   `trucks1`.`number`, `routes1`.`name`, '  :', `routes1`.`time`), '') as 'truck', IF(    CHAR_LENGTH(`slots1`.`name`), CONCAT_WS('',   `slots1`.`name`), '') as 'slot', IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS('',   `routes1`.`amount`), '') as 'amount', IF(    CHAR_LENGTH(if(`availability1`.`date`,date_format(`availability1`.`date`,'%m/%d/%Y'),'')), CONCAT_WS('',   if(`availability1`.`date`,date_format(`availability1`.`date`,'%m/%d/%Y'),'')), '') as 'date', IF(    CHAR_LENGTH(`availability1`.`time`), CONCAT_WS('',   `availability1`.`time`), '') as 'time', concat('<i class=\"glyphicon glyphicon-', if(`bookings`.`luggage`, 'check', 'unchecked'), '\"></i>') as 'luggage', if(`bookings`.`date_booked`,date_format(`bookings`.`date_booked`,'%m/%d/%Y'),'') as 'date_booked' FROM `bookings` LEFT JOIN `customers` as customers1 ON `customers1`.`id`=`bookings`.`id_number` LEFT JOIN `availability` as availability1 ON `availability1`.`id`=`bookings`.`truck` LEFT JOIN `trucks` as trucks1 ON `trucks1`.`id`=`availability1`.`truck` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability1`.`route` LEFT JOIN `slots` as slots1 ON `slots1`.`id`=`bookings`.`slot` "
				),
				'truck' => array(   
