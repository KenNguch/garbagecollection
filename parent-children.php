		...
		$pcConfig = array(
			...
			'availability' => array(   
				'truck' => array(   
					...
					'forced-where' => '',
					'display-fields' => array(1 => 'truck', 2 => 'Route', 3 => 'Amount', 4 => 'Date', 5 => 'Departure Time', 6 => 'Status'),
					'display-field-names' => array(1 => 'truck', 2 => 'route', 3 => 'amount', 4 => 'date', 5 => 'time', 6 => 'status'),
					'sortable-fields' => array(0 => '`availability`.`id`', 1 => '`truckes1`.`number`', 2 => 3, 3 => '`routes1`.`amount`', 4 => '`availability`.`date`', 5 => '`availability`.`time`', 6 => 7),
					'records-per-page' => 10,
					'default-sort-by' => 0,
					...
					'query' => "SELECT `availability`.`id` as 'id', IF(    CHAR_LENGTH(`truckes1`.`number`), CONCAT_WS('',   `truckes1`.`number`), '') as 'truck', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS('',   `routes1`.`name`, '  :', `routes1`.`time`), '') as 'route', IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS('',   `routes1`.`amount`), '') as 'amount', if(`availability`.`date`,date_format(`availability`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`availability`.`time`, '%r') as 'time', `availability`.`status` as 'status' FROM `availability` LEFT JOIN `trucks` as truckes1 ON `truckes1`.`id`=`availability`.`truck` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` "
				),
				'route' => array(   
					'parent-table' => 'routes',
					'parent-primary-key' => 'id',
			...
			'bookings' => array(   
				'id_number' => array(   
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Id number', 2 => 'Fullname', 3 => 'Phone', 4 => 'truck', 5 => 'slot', 6 => 'Amount', 7 => 'Date', 8 => 'Departure Time', 9 => 'Luggage', 10 => 'Date booked'),
					'display-field-names' => array(1 => 'id_number', 2 => 'fullname', 3 => 'phone', 4 => 'truck', 5 => 'slot', 6 => 'amount', 7 => 'date', 8 => 'time', 9 => 'luggage', 10 => 'date_booked'),
					'sortable-fields' => array(0 => '`bookings`.`id`', 1 => '`customers1`.`id_number`', 2 => '`customers1`.`fullname`', 3 => '`customers1`.`phone`', 4 => 5, 5 => '`slots1`.`name`', 6 => 7, 7 => 'date_format(`availability1`.`date`,\'%m/%d/%Y\')', 8 => '`availability1`.`time`', 9 => 10, 10 => '`bookings`.`date_booked`'),
					'records-per-page' => 10,
					'default-sort-by' => 0,
					'default-sort-direction' => 'desc',
					'open-detail-view-on-click' => true,
					...
					'query' => "SELECT `bookings`.`id` as 'id', IF(    CHAR_LENGTH(`customers1`.`id_number`), CONCAT_WS('',   `customers1`.`id_number`), '') as 'id_number', IF(    CHAR_LENGTH(`customers1`.`fullname`), CONCAT_WS('',   `customers1`.`fullname`), '') as 'fullname', IF(    CHAR_LENGTH(`customers1`.`phone`), CONCAT_WS('',   `customers1`.`phone`), '') as 'phone', IF(    CHAR_LENGTH(`truckes1`.`number`) || CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS('',   `truckes1`.`number`, `routes1`.`name`, '  :', `routes1`.`time`), '') as 'truck', IF(    CHAR_LENGTH(`slots1`.`name`), CONCAT_WS('',   `slots1`.`name`), '') as 'slot', IF(    CHAR_LENGTH(`routes1`.`amount`), CONCAT_WS('',   `routes1`.`amount`), '') as 'amount', IF(    CHAR_LENGTH(if(`availability1`.`date`,date_format(`availability1`.`date`,'%m/%d/%Y'),'')), CONCAT_WS('',   if(`availability1`.`date`,date_format(`availability1`.`date`,'%m/%d/%Y'),'')), '') as 'date', IF(    CHAR_LENGTH(`availability1`.`time`), CONCAT_WS('',   `availability1`.`time`), '') as 'time', concat('<i class=\"glyphicon glyphicon-', if(`bookings`.`luggage`, 'check', 'unchecked'), '\"></i>') as 'luggage', if(`bookings`.`date_booked`,date_format(`bookings`.`date_booked`,'%m/%d/%Y'),'') as 'date_booked' FROM `bookings` LEFT JOIN `customers` as customers1 ON `customers1`.`id`=`bookings`.`id_number` LEFT JOIN `availability` as availability1 ON `availability1`.`id`=`bookings`.`truck` LEFT JOIN `trucks` as truckes1 ON `truckes1`.`id`=`availability1`.`truck` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability1`.`route` LEFT JOIN `slots` as slots1 ON `slots1`.`id`=`bookings`.`slot` "
				),
				'truck' => array(   
