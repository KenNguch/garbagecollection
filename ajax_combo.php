	...
	$lookups = array(   
		...
		'bookings' => array(   
			...
			'truck' => array(
				...
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`availability`.`truck`) || CHAR_LENGTH(`availability`.`route`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`trucks1`.`number`), CONCAT_WS(\'\',   `trucks1`.`number`), \'\'), \'   |:|\', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS(\'\',   `routes1`.`name`, \'  :\', `routes1`.`time`), \'\')), \'\')',
				'parent_from' => '`availability` LEFT JOIN `trucks` as trucks1 ON `trucks1`.`id`=`availability`.`truck` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `availability`.`id`, IF(CHAR_LENGTH(`availability`.`truck`) || CHAR_LENGTH(`availability`.`route`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`trucks1`.`number`), CONCAT_WS(\'\',   `trucks1`.`number`), \'\'), \'   :\', IF(    CHAR_LENGTH(`routes1`.`name`) || CHAR_LENGTH(`routes1`.`time`), CONCAT_WS(\'\',   `routes1`.`name`, \'  :\', `routes1`.`time`), \'\')), \'\') FROM `availability` LEFT JOIN `trucks` as trucks1 ON `trucks1`.`id`=`availability`.`truck` LEFT JOIN `routes` as routes1 ON `routes1`.`id`=`availability`.`route` WHERE `availability`.status=\'available\' ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => false
			),
