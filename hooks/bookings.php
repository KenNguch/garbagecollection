<?php
	...
	function bookings_before_insert(&$data, $memberInfo, &$args){
		...
		if ($actualchosenidno==$getactualidno) {
			...
			$truckid=($data['truck']);
			$slotid=($data['slot']);
			#check if the selected slot in the truck is already booked or not
			$countquery=sqlValue("SELECT COUNT(*) FROM bookings WHERE truck='$truckid' AND slot='$slotid' ORDER BY id");
		if ($countquery>0) {
			# code...if slot is booked display error message
			$_SESSION['custom_err_msg']="<b>Sorry,it seems the slot you have selected has alredy been booked in that truck.Please Select another slot</b>";
			return FALSE;
		}
		#code..check for double booking for same customer/truck/date/time
		$customerid=($data['id_number']);
		$truckid=($data['truck']);
		$checkvalues=sqlValue("SELECT COUNT(*) FROM bookings WHERE id_number='$customerid' AND truck='$truckid'");
		if ($checkvalues>0) {
			# code...
			# code...if there is double bookinh show error
			$_SESSION['custom_err_msg']="<b>Sorry,it seems you are doing double booking for the same customer,date,truck and departure time,please check and try again</b>";
			return FALSE;
		}
		else{
			#if slot is not booked then save the record
			return TRUE;}
		}
		else{
